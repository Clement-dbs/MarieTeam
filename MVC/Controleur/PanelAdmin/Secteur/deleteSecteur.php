...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!empty($_POST['id_secteur'])) {
        $idSecteur = intval($_POST['id_secteur']);

        try {
            $pdo = connexionDatabase();
            $query = $pdo->prepare("DELETE FROM secteur WHERE id = :id");
            $query->bindParam(':id', $idSecteur, PDO::PARAM_INT);
...
header("Location: ./?action=panelSecteur");